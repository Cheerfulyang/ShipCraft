...
class Shell(pygame.sprite.Sprite):
    def __init__(self, Shell_surface, distance, direction, Shell_init_pos):
        ...
        self.origin_image = Shell_surface
        self.range = distance
    ...
    def update(self):
        if self._direction == 0:
            ...
            #rotate
            delta_x = ( self.pos.getx() - self.old_pos.getx() )
            delta_y = ( self.pos.gety() - self.old_pos.gety() )
            print("%d, %d" %(delta_x, delta_y))
            rangle = math.atan2(delta_y, -delta_x)
            rangled = wrap_angle( -math.degrees(rangle) )
            print("rangle %d" %rangle)
            self.image = pygame.transform.rotate(self.origin_image, rangled)          
            self.old_pos.setx(self.pos.getx())
            self.old_pos.sety(self.pos.gety())
            #end rotate
        ...
        if self._direction == 1:
            ...
            if self.rect.left > self.init_pos[0]+self.range:                    
                self.image = blank_surface
                screen.blit(boom_surface[self.down_index], (self.init_pos[0]+self.range, self.init_pos[1]))
                if self.down_index < 6:
                    self.down_index += 1
                else:
                    self.kill()
...
shell_switch2 = 0
while True:
    clock.tick(100) #Limit the maximum number of frames
